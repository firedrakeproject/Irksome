...
from ufl.classes import Zero, Form, Integral, Expr
...
def map_integrands_filtered(function, form, only_integral_type=None):
    ...
    if isinstance(form, Form):
        mapped_integrals = [map_integrands_filtered(function,
                                                    itg,
                                                    only_integral_type)
        ...
        itg = form
        if (only_integral_type is None) \
           or (itg.integral_type() in only_integral_type):
            form_integrand, keepme = function(itg.integrand())
            if keepme:
                return itg.reconstruct(form_integrand)
            else:
                return itg.reconstruct(
                    Zero(shape=form_integrand.ufl_shape,
...
def map_integrand_dags_filtered(function, form,
                                only_integral_type=None,
                                compress=True):
    return map_integrands_filtered(
        lambda expr: map_expr_dag(function, expr, compress),
...
class TimeTermsKiller(MultiFunction):
    ...
    def time_derivative(self, o):
        return ufl.classes.Zero(o.ufl_shape,
                                o.ufl_free_indices,
