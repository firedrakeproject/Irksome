...
from fetsome.galerkin_in_time.burgers.burgers import solve_burgers
from argparse import ArgumentParser

import numpy as np

# Parser setup to run the script
parser = ArgumentParser("python3 burgers_graph.py", description="Solve and plot the 1d Burgers equation with continuous "
                        "spatial elements (unstable)")
...
if __name__ == "__main__":
    ...
    us = solve_burgers(Ns, dt, tmax, kt, generator_code, info=True)
    ...
    for i in range(plotsnum):
        for j in range(plotsnum):
            ...
            plot(us[plotsnum*i + j], axes=axes[i, j], color="green", linewidth=0.8)
