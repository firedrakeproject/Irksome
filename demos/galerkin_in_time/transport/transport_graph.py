...
from fetsome.galerkin_in_time.transport.transport import solve_transport
from argparse import ArgumentParser

import numpy as np

# Parser setup to run the script
parser = ArgumentParser("python3 forced_graph.py", description="Solve and plot the 1d forced heat equation with "
                        "time boundary term")
...
if __name__ == "__main__":
    ...
    us = solve_transport(Ns, dt, tmax, kt, generator_code, info=True)
    ...
    horiz_ticks = np.linspace(0., 10., 11)
    vert_ticks = np.linspace(0, 6, 7)
