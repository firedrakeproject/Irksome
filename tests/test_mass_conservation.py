...
from firedrake import (
    CellVolume, DirichletBC, FacetArea, FacetNormal, Function,
    FunctionSpace, TestFunction, UnitIntervalMesh, UnitSquareMesh,
    assemble, avg, ds, dS, dx, exp, grad, inner, jump,
)
from irksome import (BackwardEuler, Dt, GaussLegendre, MeshConstant,
...
def test_composite_dt_requires_stiffly_accurate():
    """Non-stiffly-accurate methods with Dt(g(u)) should raise."""
    with pytest.raises(NotImplementedError, match="stiffly accurate"):
        run_richards(GaussLegendre(2), "value")


def test_standard_dt_stage_value():
    """Standard Dt(u) with stage_type='value' still works (regression)."""
    ...
    stepper = TimeStepper(F, BackwardEuler(), t, dt, u,
                          bcs=bc, stage_type="value",
                          solver_parameters={"snes_type": "ksponly",
                                             "ksp_type": "preonly",
                                             "pc_type": "lu"})
    stepper.advance()
    t.assign(float(t) + float(dt))
    # Reaching here without error means the standard Dt(u) path works
